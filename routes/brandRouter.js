...
async function handleFindMaxMinModel(pAction) {
  ...
  const { arrayModelsBrand, arrayTotalModels } = await handleCarList();
  console.log(arrayModelsBrand, arrayTotalModels);
  maxMinModel =
    pAction === "max"
      ? Math.max(...arrayTotalModels)
      : Math.min(...arrayTotalModels);
...
async function handleFindModels(pAction, pQtd) {
  let { arrayModelsBrand } = await handleCarList(),
    vQtdModels = parseInt(pQtd),
    count = 0,
    vResult = [],
    container = {};

  let arrayModelsBrandSorted =
    pAction === "mais"
...
function handleBrandModels(pReq) {
  ...
  let vModels = brands.filter(
    (brands) => brands.brand.toLowerCase() === vBrand.toLowerCase()
  );
